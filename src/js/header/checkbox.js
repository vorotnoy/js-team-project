...
function checkBtn() {
  if (checkboxOn.checked) {
    ...
    clrwh.forEach(item => {item.classList.toggle('js-colorwhite')});
    clror.forEach(item => {item.classList.toggle('js-colororange') });
    clrgr.forEach(item => {item.classList.toggle('js-colorgray')});
  } else {
    bodyOn.classList.toggle('js-bgcblack');
    bgclr.classList.toggle('js-bgcblack');
    clrwh.forEach(item => {item.classList.toggle('js-colorwhite')});
